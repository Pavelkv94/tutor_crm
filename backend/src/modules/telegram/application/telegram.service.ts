...
import { TELEGRAM_MESSAGES } from '@/modules/telegram/telegram.messages';
...
export class TelegramService extends Telegraf {
	...
	constructor(
		@Inject(telegramConfig.KEY) private readonly telegramConfig: TelegramConfig,
	...
	async onStart(@Ctx() ctx: any) {
		...
		if (!token && chatId !== +this.telegramConfig.adminId) {
	...
	async generateTelegramLink(telegramLinkInputDto: TelegramLinkInputDto): Promise<TelegramLinkOutputDto> {
		...
		return { link: `https://t.me/${this.telegramConfig.botName}?start=${telegramToken.token}` };
	}
