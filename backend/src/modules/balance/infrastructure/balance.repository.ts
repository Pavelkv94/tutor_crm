	...
	async createPayment(tx: Prisma.TransactionClient, data: CreatePaymentData): Promise<PaymentEntity> {
		const payment = await tx.payment.create({
			data: {
				...
				charge_currency: data.charge_currency ?? null,
				charge_amount_minor: data.charge_amount_minor ?? null,
	...
	async updatePayment(tx: Prisma.TransactionClient, paymentId: number, data: UpdatePaymentData): Promise<PaymentEntity> {
		const payment = await tx.payment.update({
			...
			data: {
				...
				...(data.charge_currency !== undefined ? { charge_currency: data.charge_currency } : {}),
				...(data.charge_amount_minor !== undefined ? { charge_amount_minor: data.charge_amount_minor } : {}),
				...(data.charge_rate !== undefined ? { charge_rate: data.charge_rate } : {}),
