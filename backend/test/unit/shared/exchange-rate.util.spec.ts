...
describe("exchange-rate.util", () => {
	describe("bynToEurMinor", () => {
		it("converts a whole rate exactly", () => {
			// 4 занятия по 20 BYN при курсе 5.00 дают ссылку на 16.00 €.
			expect(bynToEurMinor(20, 500)).toBe(400);
			expect(bynToEurMinor(20, 500) * 4).toBe(1600);
		});

		it("rounds half up to the cent", () => {
			// 20 / 3.30 = 6.0606…
			expect(bynToEurMinor(20, 330)).toBe(606);
			// 25 / 3.30 = 7.5757…
			expect(bynToEurMinor(25, 330)).toBe(758);
		});
		...
		it("keeps the sum of items equal to the invoice charge", () => {
			...
			const perLesson = bynToEurMinor(25, 330);
